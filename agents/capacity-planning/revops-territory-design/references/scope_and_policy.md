...
Return `POLICY_REQUIRED` when purpose, policy version, owner, or prohibited uses are absent. Return `SOURCE_REQUIRED` when a named snapshot cannot be reproduced. A request to "make territories fair" or "optimize coverage" is not a testable policy; require explicit candidate assignments and customer-owned constraints.

The skill validates evidence only. It cannot define business strategy, decide what fairness means, or authorize a change.
