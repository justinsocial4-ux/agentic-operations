...
8. Use plain Decimal strings with at most 28 total digits excluding the decimal point and approved scale from 0 through 6. Reject oversized values, binary floats, non-finite values, negative amounts, zero candidate quota, mixed currency, mixed period, mixed amount basis, mixed territory basis, or mixed coverage basis. Let only the helper's explicit local calculation context perform arithmetic.
