#!/usr/bin/env python3
"""Deterministic, read-only competitive price evidence review."""
...
BOUNDARY = "NO PRICE OR DISCOUNT RECOMMENDATION / NO MONITORING, ALERT, CRM, CUSTOMER, OR WORKFORCE ACTION"
PURPOSE = "competitive_price_evidence_review"
LANES = {"public_list", "supplied_quote", "transcript_code", "review_code", "internal_offer"}
REQUIRED_PROHIBITED = {"price-recommendation", "discount-recommendation", "monitoring", "alert", "crm-write", "customer-action", "workforce-action"}
...
def review_price_evidence(document: dict[str, Any]) -> dict[str, Any]:
    ...
    _keys(policy, {"policy_id", "policy_version", "owner_id", "human_reviewer_role_id", "effective_at", "expires_at", "cutoff_at", "timezone", "approved_purpose", "prohibited_uses", "correction_path", "calculation_policy", "source_bindings", "comparison_bases"}, "policy")
    ...
    for index, raw in enumerate(_arr(policy["source_bindings"], "policy.source_bindings")):
        ...
        _keys(row, {"lane", "source_id", "schema_version", "authorization_id", "authorization_effective_at", "authorization_expires_at"}, f"source_bindings[{index}]")
        if row["lane"] not in LANES:
            raise ValueError("source lane is unsupported")
        for field in ("source_id", "authorization_id"):
    ...
    basis_fields = {"basis_id", "basis_version", "product_scope_id", "configuration_scope_id", "quantity", "unit", "currency", "price_kind", "billing_basis", "billing_period", "contract_term", "region", "tax_basis", "fee_basis", "discount_basis"}
    for index, raw in enumerate(_arr(policy["comparison_bases"], "policy.comparison_bases")):
        ...
        for field in ("basis_id", "product_scope_id", "configuration_scope_id"):
            _id(row[field], f"basis.{field}")
        for field in basis_fields - {"basis_id", "product_scope_id", "configuration_scope_id", "quantity"}:
    ...
    observation_fields = {"observation_id", "lane", "party_id", "item_id", "source_id", "schema_version", "authorization_id", "basis_id", "basis_version", "amount", "effective_begin", "effective_end", "occurred_at", "captured_at", "population_receipt_id"}
    for index, raw in enumerate(_arr(root["observations"], "observations")):
        ...
        for field in ("observation_id", "party_id", "item_id", "source_id", "authorization_id", "basis_id", "population_receipt_id"):
        ...
        if begin >= end or occurred > captured or captured > cutoff or captured < auth_effective:
    ...
    return {
        ...
        "review_type": "COMPETITIVE_PRICE_EVIDENCE_REVIEW",
...
def main(argv: list[str]) -> int:
    try:
        if len(argv) > 2:
            raise ValueError("usage: price_evidence.py [input.json]")
        ...
        sys.stderr.write(f"price evidence review failed: {exc}\n")
        return 2
